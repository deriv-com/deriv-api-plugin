...
Update the authenticated client's payment agent privacy preference

**Endpoint:** `PATCH /payment-agents/v1/clients/me`
...
- Request schema: [`payment_agent_client_settings_update_request.schema.json`](https://developers.deriv.com/schemas/payment_agent_client_settings_update_request.schema.json)
- Response schema: [`payment_agent_client_settings_update_response.schema.json`](https://developers.deriv.com/schemas/payment_agent_client_settings_update_response.schema.json)

## Example

```bash
curl -X PATCH "https://api.derivws.com/payment-agents/v1/clients/me" \
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `data` | object | Yes | Settings to update. |

### `data`

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `show_real_name` | boolean | Yes | New value for the privacy preference. `true` reveals the client's real name to payment agents on future transactions; `false` withholds it. |

## Response

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `data` | object | Yes | The client's updated settings for using payment agents. |
...
_Source: [payment-agent-client-settings-update](https://developers.deriv.com/llms/payment-agent-client-settings-update.md)._
