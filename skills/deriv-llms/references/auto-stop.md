...
Stop an automated trading run. No further contracts will be purchased; contracts already open continue to run.
...
- Request schema: [`auto_stop_request.schema.json`](https://developers.deriv.com/schemas/auto_stop_request.schema.json)
- Response schema: [`auto_stop_response.schema.json`](https://developers.deriv.com/schemas/auto_stop_response.schema.json)
...
|-------|------|----------|-------------|
| `auto_stop` | integer: [1] | Yes | Must be `1` |
| `run_id` | string, pattern ^[\w-]{1,128}$ | Yes | The run ID to stop. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `auto_stop` | object | No | Updated details of the stopped automated trading run |
| `echo_req` | object | Yes | Echo of the request made. |
| `msg_type` | string: ["auto_stop"] | Yes | Action name of the request made. |
...
| `stop_reason` | string: ["error","user_stopped","condition_triggered"] | Yes | Reason the run was stopped |
| `stop_reason_code` | string | No | Code associated with the stop reason (absent if still running or paused) |
| `stop_time` | integer | Yes | Epoch of when the run was stopped |
...
_Source: [auto-stop](https://developers.deriv.com/llms/auto-stop.md)._
