...
Retrieve details for a specific automated trading run, with optional subscription to real-time updates
...
- Request schema: [`auto_get_request.schema.json`](https://developers.deriv.com/schemas/auto_get_request.schema.json)
- Response schema: [`auto_get_response.schema.json`](https://developers.deriv.com/schemas/auto_get_response.schema.json)
...
|-------|------|----------|-------------|
| `auto_get` | integer: [1] | Yes | Must be `1` |
| `run_id` | string, pattern ^[\w-]{1,128}$ | Yes | The run ID to retrieve. |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `auto_get` | object | No | Details of the automated trading run |
| `subscription` | object | No | For subscription requests only. |
| `echo_req` | object | Yes | Echo of the request made. |
| `msg_type` | string: ["auto_get"] | Yes | Action name of the request made. |
...
_Source: [auto-get](https://developers.deriv.com/llms/auto-get.md)._
